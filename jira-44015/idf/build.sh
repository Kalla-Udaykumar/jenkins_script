...
# make -j\$(nproc)
make install
cp -r ${WORKSPACE}/abi/mips-binary/bin/mips_ut ${WORKSPACE}/upload/
cd ${WORKSPACE}/abi/
zip -r mips-binary.zip mips-binary
ls -lrth
cp mips-binary.zip ${WORKSPACE}/upload/
cd ${WORKSPACE}/abi/mips-binary/lib
cp -r ${WORKSPACE}/abi/mips-binary/lib/libmfdl.so libmfil.so libutils.so libjson-schema-validator.so.2.0.0 ${WORKSPACE}/upload/
